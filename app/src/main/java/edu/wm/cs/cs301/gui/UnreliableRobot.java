				...
				if (FrontSensor.distanceToObstacle(getCurrentPosition(), dToCD(direction), Battery) == -1) {
					...
					if (LeftSensor.distanceToObstacle(getCurrentPosition(), dToCD(direction), Battery) == -1) {
						...
						if (RightSensor.distanceToObstacle(getCurrentPosition(), dToCD(direction), Battery) == -1) {
							...
							rotate(Turn.LEFT);
							distance = RightSensor.distanceToObstacle(getCurrentPosition(), dToCD(Direction.LEFT), Battery);
							rotate(Turn.RIGHT);
							break switcher;
						}
					}
						...
						rotate(Turn.RIGHT);
						distance = LeftSensor.distanceToObstacle(getCurrentPosition(), dToCD(Direction.RIGHT), Battery);
						rotate(Turn.LEFT);
						break switcher;
					}
				}
		...
		case LEFT:
			if (LeftSensor == null) {
				...
				if (LeftSensor.distanceToObstacle(getCurrentPosition(), dToCD(direction), Battery) == -1) {
					...
					return LeftSensor.distanceToObstacle(getCurrentPosition(), dToCD(Direction.RIGHT), Battery); } //Lol
			}
			break switcher;
		//--------------------------------------------------
		case RIGHT:
